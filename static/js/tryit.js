    ...
    for (i = 0; i < sURLVariables.length; i++) {
      ...
      if (sParameterName[0] === sParam) {
        ...
        if (sParameterName[1].substr(-1) === "/") {
          sParameterName[1] = sParameterName[1].substr(
            0,
            sParameterName[1].length - 1
          );
        }
  ...
  $("#tryit_intro").on("shown.bs.collapse", function (e) {
    ...
    if (offset) {
      $("html,body").animate(
        {
          scrollTop:
            $(".panel-collapse.in").siblings(".panel-heading").offset().top -
            50,
        },
        500
      );
